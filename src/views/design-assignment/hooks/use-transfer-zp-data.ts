...
import { useDesignSpecProvReport } from './api'
import { useFiltersStore } from '@/shared/store'
import { ALL_ROADS } from '@/shared/lib/const'
import { roads } from '@/shared/lib/data'
import { DesignSpecProvReportItem } from '../lib/types'

export function useTransferZpData() {
  ...
  // Обработка данных design_spec_prov_report
  const transferZpData = useMemo(() => {
    if (
      !reportData?.contents ||
      ...
      return {
        ...
        prevPeriodProvidedQuantity: 0,
        weeklyIncrements: [0, 0, 0, 0, 0, 0, 0, 0],
      }
    }

    // API возвращает один объект с агрегированными данными
    const data: DesignSpecProvReportItem = reportData.contents[0]

    return {
      providedQuantity: data.approved_quantity || 0,
      ...
      prevPeriodProvidedQuantity: data.prev_period_appr_quantity || 0,
      weeklyIncrements: [
        data.week1_incr || 0,
        data.week2_incr || 0,
        data.week3_incr || 0,
